...
top 명령어를 통해 서버의 CPU 사용량, 메모리 사용량 등을 확인할 수 있으며, 서버에서 구동 중인 모든 어플리케이션들을 CPU 사용률이나 메모리 사용률이 많은 순서대로 나열하여 모니터링이 가능한 명령어이다.


* #### 사용법

사용법은 간단하다. 리눅스에서 top 명령어를 치고 들어가면 된다. 리눅스 서버에서 top 명령어를 입력하면 다음과 같은 결과를 얻을 수 있다.
