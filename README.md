...
+ [top 명령어](#%EF%B8%8F-top-명령어)

+ [ps 명령어](#%EF%B8%8F-ps-명령어)

+ [jobs 명령어](#%EF%B8%8F-jobs-명령어)

+ [kill 명령어](#%EF%B8%8F-kill-명령어)

+ [vim 매크로](#vim-에디터에서-매크로-활용방법)
...
우선 vim 매크로를 시작하기 전에 vim에 대해 알아보자 
...
그럼 본격적으로 vim 매크로 기능에 대해 알아보겠다.
**사용법**

```
1) `q[name]` 로 매크로 기록 시작
2) `q`로 매크로 기록 종료
3) `@[name]` 로 저장된 매크로 실행
4) `[number]@[name]`로 매크로 여러번 실행
```

### 2) q[name] 로 매크로 기록 시작 `q`로 매크로 기록 종료
...
위 그림처럼 qt에 dd를 눌러 9번 라인을 제거후 @t를 눌러 저장한 매크로를 실행시켜 8번 라인을 없앴음을 확인 할 수 있다.
...
위 두 사진은 `qt`를 통해 라인 지우기를 매크로에 저장한 매크로를 `2@t`를 활용하여 연속으로 매크로가 2번 실행하게 하였다.
...
또한 vim 에디터의 매크로 경우 이 기능을 활용하여 복잡하거나 혹은 유용한 기능을 하는 키의 조합들을 저장하여 필요에 따라 바로 꺼내 사용 할 수 있다. 
